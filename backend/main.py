    ...
    for city, file in [("manhattan", "graph_manhattan.pkl"), ("detroit", "graph_detroit.pkl")]:
        ...
        if os.path.exists(path):
            try:
                ...
                print(f"{city} graph loaded with {len(graphs[city].nodes)} nodes and {len(graphs[city].edges)} edges.")
            except Exception as e:
                print(f"Error loading {city} graph: {e}")
...
def get_routes(req: RouteRequest):
    ...
    if not G or not engine:
        raise HTTPException(status_code=500, detail=f"Graph not loaded for {city}")
    ...
    try:
        ...
        # Fallback if osmnx isn't working for nearest_nodes
    ...
    # Identify anchors (just random nodes on the safe route for demo purposes)
    anchors = []
    if tiers["tier2_safe"]:
        import random
        ...
        for i in range(1, len(path)-1):
            if random.random() < 0.1: # 10% chance
                ...
                anchors.append({
                    "lat": node_data['y'],
                    "lon": node_data['x'],
                    "type": random.choice(["24hr Pharmacy", "Police Booth", "Well-lit Dhaba"])
                })
